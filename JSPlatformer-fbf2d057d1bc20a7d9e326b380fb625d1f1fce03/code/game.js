...
Level.prototype.playerTouched = function(type, actor) {
  ...
  //make portal disapear when used
  else if (type == 'portal') {
    this.player.pos = new Vector(45,0)
  }
  //interaction for new skip element
  else if (type == "skip") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    // If there aren't any skips left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "skip";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
